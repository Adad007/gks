...
async def next_page(bot, query):
    ...
    btn.insert(0,
        [
            InlineKeyboardButton(text="⚡ʜᴏᴡ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ⚡", url='https://t.me/+EARXv7Hz4Uo4Mjll')
        ]
    )
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("wait akk buddy", show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🕸️ Movie Updates 🕸️', url=f"https://t.me/admoviesltd"),
            InlineKeyboardButton('🌀 BOT Updates ☄️', url='https://t.me/admoviesltd')
            ],[
        ...
        buttons = [[
            ...
            InlineKeyboardButton('⛩️ H𝙾𝙼𝙴 ⛩️', callback_data='start'),
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
